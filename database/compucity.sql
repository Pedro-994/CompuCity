...
DELIMITER //
CREATE TRIGGER NUMEROPEDIDO BEFORE UPDATE ON DETPEDIDO FOR EACH ROW
BEGIN
	UPDATE USUARIO SET NUMPEDIDO = NUMPEDIDO+1 WHERE IDUSUARIO = (SELECT IDUSUARIO FROM PEDIDO 
    WHERE IDPEDIDO = (SELECT IDPEDIDO FROM DETPEDIDO WHERE NEW.ESTATUS="Entregado"));
END //
